...
- [x] 5.1 Update frontend to display MCP_SERVICE_UNAVAILABLE errors
- [x] 5.2 Add error notification component for MCP service errors
- [x] 5.3 Test error display in streaming and non-streaming modes

## 6. Testing & Migration

- [x] 6.1 Create unit tests for MCPClient
- [x] 6.2 Create unit tests for MCP Service endpoints
- [x] 6.3 Create integration tests for MCP communication
- [x] 6.4 Verify parallel running with current architecture

### 6.5 Parallel Run Verification

- [x] 6.5.1 Define test criteria: latency diff < 5%, coverage = 100%, error rate < 0.1%
- [x] 6.5.2 Start current architecture (ErpClient direct call to erp_app)
- [x] 6.5.3 Start MCP Service and configure ClientFactory to point to new endpoint
- [x] 6.5.4 Execute functional comparison test script
- [x] 6.5.5 Record latency and error rate
- [x] 6.5.6 Switch configuration after verification passes

### 6.6 Rollback Procedure

- [x] 6.6.1 Define trigger conditions: coverage < 99%, error rate > 1%, latency diff > 10%
- [x] 6.6.2 Set `CLIENT_BACKEND=erp_adapter` in environment to switch back
- [x] 6.6.3 Restart Agent service
- [x] 6.6.4 Verify old interface works correctly
- [x] 6.6.5 Keep MCP Service for subsequent debugging