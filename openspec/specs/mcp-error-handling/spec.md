...
### Requirement: MCP Error Recovery
The system SHALL support automatic recovery from recoverable MCP errors.

#### Scenario: Recoverable error indication
- **GIVEN** a recoverable MCP error occurs (MCP_SERVICE_UNAVAILABLE, MCP_CONNECTION_TIMEOUT, MCP_INVALID_RESPONSE)
- **WHEN** the error is displayed
- **THEN** the message SHALL indicate the error is temporary with recoverable=true

#### Scenario: Non-recoverable error indication
- **GIVEN** a non-recoverable MCP error occurs (MCP_AUTH_FAILED, MCP_TOOL_NOT_FOUND)
- **WHEN** the error is displayed
- **THEN** the message SHALL indicate the error requires configuration fix with recoverable=false